...
public class LoadingHudOverlay {
    ...
    public static final IGuiOverlay LOADING_HUD = ((gui, guiGraphics, partialTick, screenWidth, screenHeight) -> {
        ...
        if (player.getPersistentData().getBoolean(Main.MOD_ID + "_isLoading")) {
            System.out.println(player.getPersistentData().getInt(Main.MOD_ID + "_loading_time"));
            player.getPersistentData().putInt(Main.MOD_ID + "_loading_time", player.getPersistentData().getInt(Main.MOD_ID + "_loading_time") + 1);
            if (player.getPersistentData().getInt(Main.MOD_ID + "_loading_time") >= PHASE * TIMER) {
                player.getPersistentData().putInt(Main.MOD_ID + "_loading_time", 0);
            }
            int type = player.getPersistentData().getInt(Main.MOD_ID + "_loading_time") / TIMER;
