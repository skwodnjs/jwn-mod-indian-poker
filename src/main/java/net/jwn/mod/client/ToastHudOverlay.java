...
public class ToastHudOverlay {
    ...
    public static final IGuiOverlay TOAST_HUD = ((gui, guiGraphics, partialTick, screenWidth, screenHeight) -> {
        ...
        int x = -160;
        if (player.getPersistentData().getBoolean(Main.MOD_ID + "_toast")) {
            player.getPersistentData().putInt(Main.MOD_ID + "_toast_x", player.getPersistentData()
                    .getInt(Main.MOD_ID + "_toast_x") + 2);
            if (player.getPersistentData().getInt(Main.MOD_ID + "_toast_x") < TIMER) {
                ...
                if (x == -160) {
                    player.getPersistentData().putBoolean(Main.MOD_ID + "_toast", false);
