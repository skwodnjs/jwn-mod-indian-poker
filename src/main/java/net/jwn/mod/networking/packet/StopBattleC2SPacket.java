...
public class StopBattleC2SPacket {
    ...
    public boolean handle(Supplier<NetworkEvent.Context> supplier) {
        ...
        context.enqueueWork(() -> {
            ...
            player.sendSystemMessage(Component.translatable("message." + Main.MOD_ID + ".stop_battle"));
            if (target != null) {
                target.getPersistentData().remove(Main.MOD_ID + "_opponent");
                target.getPersistentData().putBoolean(Main.MOD_ID + "_battle", false);
                target.sendSystemMessage(Component.translatable("message." + Main.MOD_ID + ".stop_battle"));
            }
        });
        return true;
    }
