    ...
    private int getOpponent(CommandContext<CommandSourceStack> command, Collection<? extends Entity> pTargets) {
        if (command.getSource().getEntity() instanceof Player player) {
            if (pTargets.size() != 1) {
                ...
                player.sendSystemMessage(Component.translatable("error." + Main.MOD_ID + ".opponent_empty"));
            } else {
                // 지목한 상대가 있어? 그러면 알려줄게.
                ...
                if (opponent == null) {
                    ...
                    player.sendSystemMessage(Component.translatable("message." + Main.MOD_ID + ".get_opponent_not_in_game"));
                } else {
                    player.sendSystemMessage(
                            Component.translatable("message." + Main.MOD_ID + ".get_opponent",
